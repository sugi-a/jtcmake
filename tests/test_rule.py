...
@pytest.mark.parametrize("xs,xisorig,expect", [
...
@pytest.mark.parametrize("dry_run,par_updated,expect", [
...
@pytest.mark.parametrize("ys,xs,xisvf,expect", [
    (
        [EPath(), EPath()],
        [EPath(1), EPath(-1)],
        [False, False],
...
@pytest.mark.parametrize("memo,old_memo_file,expect", [
...
def test_postprocess(tmp_path: Path, mocker: Any):
    ...
    r = Rule(
        [y], [x1, x2], xisorig, [True, False], set(), _method, _args, _kwargs, mock_memo, 0
    )
